...
func TestStoreCanceledJobs(t *testing.T) {
	...
	toCancel, err := testStore(db, defaultTestStoreOptions(nil, testScanRecord)).CanceledJobs(context.Background(), []int{1, 2, 3}, CanceledJobsOptions{WorkerHostname: "worker1"})
	if err != nil {
		t.Fatalf("unexpected error marking upload as completed: %s", err)
	}
