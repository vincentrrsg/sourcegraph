...
func (s *outboundWebhookStore) listQuery(opts OutboundWebhookListOpts) *sqlf.Query {
...
func (s *outboundWebhookStore) scanOutboundWebhook(webhook *types.OutboundWebhook, sc dbutil.Scanner) error {
	...
	if keyID != "" {
		...
		webhook.URL = encryption.NewUnencrypted(string(rawURL))
		webhook.Secret = encryption.NewUnencrypted(string(rawSecret))
	}
